...
void
RecordReader::readHeader()
{
    if (!d_input->read(reinterpret_cast<char*>(&d_header), sizeof(d_header))
        || (memcmp(d_header.magic, MAGIC, sizeof(MAGIC)) != 0)) {
        throw std::ios_base::failure(
                "The provided input file does not look like a binary generated by memray.");
    }
    if (d_header.version != CURRENT_HEADER_VERSION) {
        throw std::ios_base::failure(
                "The provided input file is incompatible with this version of memray.");
    }
    d_command_line.reserve(4096);
    if (!d_input->getline(d_command_line, '\0')) {
...
const std::string&
RecordReader::getCommandLine() const noexcept
{
